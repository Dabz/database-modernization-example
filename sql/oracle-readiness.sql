...
DECLARE
    ...
    g_required_sel_obj_privs varchar2_tt := varchar2_tt(
        ...
        'ALL_ENCRYPTED_COLUMNS', 'ALL_LOG_GROUPS', 'ALL_TAB_PARTITIONS', 'DBA_REGISTRY',
        'OBJ$', 'DBA_TABLESPACES', 'DBA_OBJECTS', 'ENC$'
    );
...
BEGIN
    IF g_is_db_vault_enabled AND k_db_version >= 12 AND g_pdb_name IS NULL AND NOT g_is_rds THEN
        g_required_exec_obj_privs := g_required_exec_obj_privs MULTISET UNION varchar2_tt('DBMS_LOGMNR_D', 'DBMS_LOGMNR_LOGREP_DICT', 'DBMS_LOGMNR_SESSION');
    END IF;
END;
